...
def load_local_parquet_data(symbol="BTCUSDT", interval="1d"):
    ...
    # 根據 interval 選擇不同的 parquet 文件
    if interval == "1d":
        path = Path(PROJECT_ROOT) / "data" / f"{symbol.lower()}_1d.parquet"
    elif interval == "4h":
        path = Path(PROJECT_ROOT) / "data" / f"{symbol.lower()}_4h.parquet"
    elif interval == "15m":
        path = Path(PROJECT_ROOT) / "data" / f"{symbol.lower()}_15m.parquet"
    else:
        path = Path(PROJECT_ROOT) / "data" / f"{symbol.lower()}_1d.parquet"
    ...
    local_data = None
    if args.local:
        local_data = load_local_parquet_data(args.symbol, args.interval)
        if local_data is None:
            print("⚠️ 無法載入本地數據，將使用 API 獲取")
...
def main():
    ...
    p_run.add_argument("--interval", "-i", default="1d", help="K線周期 (默認: 1d)")
    p_run.add_argument("--local", "-l", action="store_true", help="使用本地 parquet 數據")
