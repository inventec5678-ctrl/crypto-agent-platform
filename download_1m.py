#!/usr/bin/env python3
"""
Download BTCUSDT 1m data (1 year)
"""
import asyncio
import aiohttp
import pandas as pd
...
async def download():
    filepath = DATA_DIR / "btcusdt_1m.parquet"
    
    if filepath.exists():
        df = pd.read_parquet(filepath)
        print(f"[Cache] btcusdt_1m.parquet already exists: {len(df)} rows")
        return df
    
    print("Downloading BTCUSDT 1m data (1 year)...")
    ...
    async with aiohttp.ClientSession() as session:
        while current_ts < end_ts:
            params = {"symbol": "BTCUSDT", "interval": "1m", "startTime": current_ts, "endTime": end_ts, "limit": 1000}
            try:
