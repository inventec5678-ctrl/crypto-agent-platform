#!/usr/bin/env python3
"""
Download BTCUSDT 4h data (3 years)
"""
import asyncio
import aiohttp
import pandas as pd
...
async def download():
    filepath = DATA_DIR / "btcusdt_4h.parquet"
    
    if filepath.exists():
        df = pd.read_parquet(filepath)
        print(f"[Cache] btcusdt_4h.parquet already exists: {len(df)} rows")
        return df
    
    print("Downloading BTCUSDT 4h data...")
    ...
    async with aiohttp.ClientSession() as session:
        while current_ts < end_ts:
            params = {"symbol": "BTCUSDT", "interval": "4h", "startTime": current_ts, "endTime": end_ts, "limit": 1000}
            try:
