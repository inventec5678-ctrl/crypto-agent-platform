...
位置: crypto-agent-platform/research_run_v2.py
"""
...
async def run_backtest(strategy_name, strategy_instance, symbol, interval,
                       start_date, end_date, tp, sl, initial_capital=10000.0):
...
async def main():
    ...
    print_result("VolMomentumBreakout_Long_v1", r1, tp=6.0, sl=3.0)
    ...
    print_result("RSIStackedBull_Long_v1", r2, tp=6.0, sl=3.0)
    
    # ---------- 實驗 3: EMAGoldenCross_4H_Long_v1 ----------
    print("\n" + "=" * 70)
    print("實驗 3: EMAGoldenCross_4H_Long_v1")
    print("數據: BTCUSDT 4H | TP=7%, SL=3%")
    print("=" * 70)
    ...
    if passed:
        ...
        print("\n😢 沒有策略達到 4/4 標準。")
        print("（按照 v2 流程，不准改參數重跑，直接進入下一輪設計）")
